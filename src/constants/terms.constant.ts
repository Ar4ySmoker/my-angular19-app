export const TERMS = [
  {
    tab: 'Terms and Conditions',
    title: 'General Rules',
    description: `We've made registering at our casino as easy as possible so you don't have to go through any hassles and formalities. Registering takes only a few seconds and then you can start enjoying many bonuses and offers at`,
    rules: [
      '1.1: The first deposit bonus is 100% up to €200 + 70 Free Spins.',
      '1.2: To receive a 100% bonus on the first deposit, players must deposit at least €20.',
      '1.3: To receive a 100% bonus and 70 Free Spins on the first deposit, players need to make a deposit of €50 or more.',
    ],
  },
  {
    tab: 'Privacy Policy',
    title: 'Create An Account',
    description: 'This section explains our privacy policy.',
    rules: [
      '2.1: Nulla quis lorem ut libero.',
      '2.2: Integer molestie lorem at massa.',
    ],
  },
  { 
    tab: 'Responsible Gaming',
    title: 'Create An Account',
    description: 'This section explains our privacy policy.',
    rules: [
      '2.1: Nulla quis lorem ut libero.',
      '2.2: Integer molestie lorem at massa.',
    ],
  },
   {
    tab: 'Bonus Terms',
    title: 'Service Account Management',
    description: 'This section explains our privacy policy.',
    rules: [
      '3.1: Nulla quis lorem ut libero.',
      '3.2: Integer molestie lorem at massa.',
    ],
  },
  {
    tab: 'Contact Us',
     title: 'Service Account Management',
    description: 'This section explains our privacy policy.',
    rules: [
      '4.1: Nulla quis lorem ut libero.',
      '4.2: Integer molestie lorem at massa.',
    ],
  }
];
